    ...
    <div className="text-white">
      ...
      <div className="">
        <div className="px-28 py-3 md:px-48">
          <h1 className="text-6xl font-impact">GDSC CLUB</h1>
        </div>
        <div className="grid grid-cols-2 z-10  md:grid-cols-4  pt-10 w-4/5 mx-auto">
          {event.map((eve) => (
            <div
              key={eve._id}
              className=" main mb-6 flex flex-col w-[250px] m-3 mx-3  p-5 bg-gray-300 rounded-2xl bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 border border-gray-100 uppercase"
            >
              <img
                ...
                alt={eve.eventName}
              />
              <Link to={`/delete/${eve._id}`}>
                <MdOutlineDelete className="text-red-600 text-3xl absolute bg-white rounded hover:bg-gray-200 hover:text-black e m-3 top-4 right-4" />
              </Link>
